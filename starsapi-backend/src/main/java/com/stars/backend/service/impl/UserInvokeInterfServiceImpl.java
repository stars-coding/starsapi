...
public class UserInvokeInterfServiceImpl extends ServiceImpl<UserInvokeInterfMapper, UserInvokeInterf>
    ...
    public boolean addInvokeCount(long interfId, long userId) {
        if (userId <= 0L || interfId <= 0L) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "次数增加失败，用户ID和接口ID不合法");
        }
        String name = RedisConstants.LOCK_INTERF_KEY + interfId;
        SimpleRedisLock lock = new SimpleRedisLock(this.redisTemplate, name);
        ...
        if (b) {
            ...
            userInvokeInterf.setLeftInvokeNum(2000L);
    ...
    public void validUserInvokeInterf(UserInvokeInterf userInvokeInterf, boolean add) {
        if (userInvokeInterf == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "校验失败，用户调用接口为空");
        }
        if (add && (userInvokeInterf.getUserId() <= 0 || userInvokeInterf.getInterfId() <= 0)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "校验失败，用户或接口不存在");
        }
        if (userInvokeInterf.getLeftInvokeNum() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "校验失败，接口剩余次数不大于1");
        }
    }
    ...
    public boolean invokeCount(long userId, long interfId) {
        // 如果用户ID小于等于零，则抛出异常
        if (userId <= 0L) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "获取失败，用户ID小于等于零");
        }
        // 如果接口ID小于等于零，则抛出异常
        if (interfId <= 0L) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "获取失败，接口ID小于等于零");
        }
        // 锁的名称
        String name = RedisConstants.LOCK_INTERF_KEY + interfId;
        // 创建Redis锁，控制并发访问
        ...
        if (lockAcquired) {
            try {
                ...
                if (count == 0) {
                    ...
                    userInvokeInterf.setInterfId(interfId);
                    // 剩余调用次数的初始值为200次
                    userInvokeInterf.setLeftInvokeNum(200L);
                    // 将【用户调用接口】保存至数据库中
                    boolean save = this.save(userInvokeInterf);
                    // 如果未保存成功，则回false
                    if (!save) {
